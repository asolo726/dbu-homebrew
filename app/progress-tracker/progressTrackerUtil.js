...
export const renderPageProgressItems = (items, depth = 0, hideCompleted = false) => {
  return Object.entries(items)
    .filter(([, value]) => !hideCompleted || !isItemCompleted(value))
    .map(([key, value]) => (
      <div key={key} style={{ marginLeft: `${depth * 1.5}rem` }}>
        ...
        {typeof value === "object" &&
          !Array.isArray(value) &&
          !("completed" in value) &&
          renderPageProgressItems(value, depth + 1, hideCompleted)}
      </div>
    ));
};
