...
export async function GET(request) {
	...
	const members = await resolveAllowlistNames(
		mainDb,
		found.doc.head.communityAllowlist ?? [],
	);
...
export async function POST(request) {
	...
	if (!keyName || !username?.trim()) {
		return NextResponse.json(
			{ error: "keyName and username required" },
			{ status: 400 },
		);
	}
...
export async function DELETE(request) {
	...
	if (!keyName || !email) {
		return NextResponse.json(
			{ error: "keyName and email required" },
			{ status: 400 },
		);
	}
	...
	const updatedList = (found.doc.head.communityAllowlist ?? []).filter(
		(e) => e !== email,
	);
	await contentDb
		.collection(found.colName)
