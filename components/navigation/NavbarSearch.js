...
export default function NavbarSearch({ fullWidth = false }) {
    ...
    return (
        <div className={`relative ${fullWidth ? "w-full" : ""}`}>
            <div className={`flex items-center gap-1 bg-neutral-700 rounded-md px-2 py-1 ${fullWidth ? "w-full" : ""}`}>
                <button onClick={submit} className="text-dbu-text/70 hover:text-dbu-text shrink-0">
                    <FiSearch size={14} />
                </button>
                <input
                    ref={inputRef}
            ...
            {open && (
                <div onMouseDown={(e) => e.preventDefault()} className="absolute top-full mt-2 right-0 bg-dbu-bg2 border border-dbu-line rounded-xl p-4 z-50 w-80 shadow-2xl">
                    <p className="text-xs text-dbu-text/50 mb-2 uppercase tracking-wider">Type</p>
                    ...
                    <p className="text-xs text-dbu-text/50 mb-2 uppercase tracking-wider">Race</p>
