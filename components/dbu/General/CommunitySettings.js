...
export default function CommunitySettings({
  ...
  const currentIsCommunity =
    pendingChanges?.["head.isCommunity"] ?? initialIsCommunity ?? false;
      ...
      {currentIsCommunity && (
        <>
          <p className="text-xs text-dbu-text/50 mb-3">
            Allowlisted users can add new contributions but cannot edit
            others&apos; entries.
          </p>
          ...
          {allowlistLoading ? (
            ...
            <p className="text-xs text-dbu-text/40">
              No users in allowlist yet.
            </p>
          ) : (
            <ul className="space-y-1.5">
              {allowlist.map(({ email, name }) => (
                <li
                  key={email}
                  className="flex items-center justify-between bg-dbu-bg2 rounded px-3 py-1.5"
                >
