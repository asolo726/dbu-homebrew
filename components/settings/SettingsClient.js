	...
	const commitChange = async (newName, updateAuthor) => {
		...
		try {
			...
			if (updateAuthor) {
				...
				if (!res2.ok)
					throw new Error(`Author update failed (${res2.status})`);
			}
			setUsername(newName);
				...
				{activeTab === "toggles" && (
					<div className="max-w-3xl mx-auto pt-2">
						...
						<p className="text-sm text-dbu-text/50 mb-6">
							Toggles control page visibility. Click a toggle name
							to manage which pages it applies to.
						</p>
						...
						{togglesLoading ? (
							...
							<p className="text-dbu-text/50 text-sm mt-8 text-center">
								You do not have any Page Toggles. Create a new
								page to make one!
							</p>
						) : (
							<>
				...
				<div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
					<div className="bg-dbu-bg2 border border-dbu-line rounded-lg p-8 max-w-sm w-full mx-4 shadow-2xl">
						...
						<p className="text-dbu-text text-sm mb-6">
							...
							. Would you like to update the Author field on those
							pages to{" "}
							<span className="text-dbu-header font-medium">
								"{pendingNewName}"
							</span>
				...
				>
					...
					>
						...
						<div className="px-6 py-4 border-b border-dbu-line">
							<h2 className="text-dbu-header font-semibold">
								{popupToggle}
							</h2>
							<p className="text-xs text-dbu-text/50 mt-0.5">
								Click a page name to move it to the other
								column. Save to apply.
							</p>
						</div>
							...
							>
								...
								<div className="px-5 py-4 border-b sm:border-b-0 border-dbu-line">
									...
									<div className="space-y-1 max-h-72 overflow-y-auto pr-1">
										{popupLeft.length === 0 ? (
											<p className="text-dbu-text/30 text-xs italic">
												None
											</p>
										) : (
											popupLeft.map((p) => (
				...
				<div className="fixed inset-0 bg-black/70 flex items-center justify-center z-60">
					<div className="bg-dbu-bg2 border border-dbu-line rounded-lg p-6 max-w-sm w-full mx-4 shadow-2xl">
						...
						<p className="text-dbu-text text-sm mb-5">
							You have unsaved changes to this toggle's page
							assignments. Close without saving?
						</p>
						<div className="flex gap-3 justify-end">
							<button
