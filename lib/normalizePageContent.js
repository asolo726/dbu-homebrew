function normalizeAddendumBox(box) {
	...
	return {
		...
		abilities: Array.isArray(box.abilities)
			...
			: [],
		traits: Array.isArray(box.traits) ? box.traits.map(normalizeTrait) : [],
	};
}
...
export function normalizePageContent(rawContent) {
	...
	normalized.data.author ??=
		normalized.head.author ?? rawContent.author ?? "";
	normalized.data.keyName ??=
		normalized.head.keyName ?? rawContent.keyName ?? "";
	normalized.data.identity ??=
		normalized.head.identity ?? rawContent.identity ?? "";
	...
	normalized.head.details.stressTest ??=
		normalized.head.details.stressTest ??
		rawContent.head?.stressTest ??
		rawContent.stressTest ??
		"";

	if (
		Array.isArray(rawContent.head?.aspects) &&
		!Array.isArray(normalized.head.details.aspects)
	) {
		normalized.head.details.aspects = rawContent.head.aspects;
	}
	if (
		Array.isArray(rawContent.head?.attributes) &&
		!Array.isArray(normalized.head.details.attributes)
	) {
		normalized.head.details.attributes = rawContent.head.attributes;
	}
	if (
		Array.isArray(rawContent.head?.attributeModifiers) &&
		!Array.isArray(normalized.head.details.attributeModifiers)
	) {
		normalized.head.details.attributeModifiers =
			rawContent.head.attributeModifiers;
	}
	if (
		!Array.isArray(normalized.head.details.aspects) &&
		Array.isArray(rawContent.aspects)
	) {
		normalized.head.details.aspects = rawContent.aspects;
	}
	if (
		!Array.isArray(normalized.head.details.attributeModifiers) &&
		Array.isArray(rawContent.attributeModifiers)
	) {
		normalized.head.details.attributeModifiers =
			rawContent.attributeModifiers;
	}
	if (
		!Array.isArray(normalized.head.details.attributeModifiers) &&
		Array.isArray(normalized.head.details.attributes)
		...
		normalized.head.details.attributeModifiers =
			normalized.head.details.attributes.map((mod) => ({
	...
	if (Array.isArray(normalized.traits)) {
		normalized.traits = normalized.traits.map((trait) =>
			normalizeTrait(trait),
		);
	}
	if (Array.isArray(normalized.masteryTrait)) {
		normalized.masteryTrait = normalized.masteryTrait.map((trait) =>
			normalizeTrait(trait),
		);
	}
	if (Array.isArray(normalized.legendaryTrait)) {
		normalized.legendaryTrait = normalized.legendaryTrait.map((trait) =>
			normalizeTrait(trait),
		);
	}
